...
def train_model(train_data, train_labels, validation_data, validation_labels):
    ...
    nb_epoch = 500
    model.fit(train_data, train_labels, validation_data=(validation_data, validation_labels), batch_size=batch_size,
              nb_epoch=nb_epoch, callbacks=callbacks, shuffle=True, verbose=1)
    return model
...
def test_on_whole_videos(train_data, train_labels, validation_data, validation_labels):
    ...
    for video_class in parent[1:]:
        print(video_class)
        child = os.listdir("C:/Users/LR/Desktop/DL_project_videos/Axel/test" + "/" + video_class)
        for class_i in child[1:]:
            sub_child = os.listdir(
                "C:/Users/LR/Desktop/DL_project_videos/Axel/test" + "/" + video_class + "/" + class_i)
            for image_fol in sub_child[1:]:
                if video_class == 'class_4':
                    if count % 4 == 0:
                        image = sp.imread(
                            "C:/Users/LR/Desktop/DL_project_videos/Axel/test" + "/" + video_class + "/" + class_i + "/" + image_fol)
                        image = sp.imresize(image, (224, 224))

                        x.append(image)
                        y.append(output)
                        cv2.imwrite(
                            'C:/Users/LR/Desktop/DL_project_videos/Axel/video/validate/' + video_class + '/' + str(
                                count) + '_' + image_fol, image)
                    count += 1

                else:
                    if count % 4 == 0:
                        image = sp.imread("C:/Users/LR/Desktop/DL_project_videos/Axel/video/test" + "/" + video_class + "/" + class_i + "/" + image_fol)
                        image = sp.imresize(image, (224, 224))
                        x.append(image)
                        y.append(output)
                        cv2.imwrite('C:/Users/LR/Desktop/DL_project_videos/Axel/video/validate/' + video_class + '/' + str(count) + '_' + image_fol,image)
                    count += 1
            correct_video+=1
            x = np.array(x)
            y = np.array(y)
